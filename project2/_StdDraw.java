 ...
 *  @author Kevin Wayne
 */
public final class _StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static _StdDraw std = new _StdDraw();
    ...
    ***************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = _StdDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
    ...
    ***************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(_StdDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            _StdDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    ***************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = _StdDraw.userX(e.getX());
            mouseY = _StdDraw.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    *  as an argument. It can handle action keys
     ...
     */
    public static void main(String[] args) {
        _StdDraw.square(.2, .8, .1);
        _StdDraw.filledSquare(.8, .8, .2);
        _StdDraw.circle(.8, .2, .2);

        _StdDraw.setPenColor(_StdDraw.BOOK_RED);
        _StdDraw.setPenRadius(.02);
        _StdDraw.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        _StdDraw.setPenRadius();
        _StdDraw.setPenColor(_StdDraw.BOOK_BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        _StdDraw.filledPolygon(x, y);

        // text
        _StdDraw.setPenColor(_StdDraw.BLACK);
        _StdDraw.text(0.2, 0.5, "black text");
        _StdDraw.setPenColor(_StdDraw.WHITE);
        _StdDraw.text(0.8, 0.8, "white text");
    }
